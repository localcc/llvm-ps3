...
public:
  PPCTargetELFStreamer(MCStreamer &S, int AbiVersion) : PPCTargetStreamer(S) {
    // todo(localcc): can't call virtual functions in constructor :(
...
createObjectTargetStreamer(MCStreamer &S, const MCSubtargetInfo &STI) {
  ...
  if (TT.isOSBinFormatELF())
    return new PPCTargetELFStreamer(
        S, STI.getTargetTriple().isPPC64ELFv2ABI() ? 2 : 1);
  if (TT.isOSBinFormatXCOFF())
